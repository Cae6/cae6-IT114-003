...
public class Problem3 {
    ...
    static <T> void bePositive(T[] arr){
        ...
        for (int i=0; i<arr.length; i++) {
            ...
            // then makes it positive if it is
            else if (arr[i] instanceof Double){
                output [i]=Math.abs((Double) arr[i]);
            } //If its not an integer, checks to see if its a double
            //then it makes it a positive
            else if (arr[i] instanceof String){
                output[i] = String.valueOf(Math.abs(Integer.parseInt((String) arr[i])));
            } // If its nether of the above, it Checks to see if its a String
            // then it converts to an int, then makes it positive
            // then turns it back into a string again
            // 9/23/24, cae6
